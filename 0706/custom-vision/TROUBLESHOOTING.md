...
6. **재게시 직후의 예측 결과를 새 모델 성능으로 단정하지 않는다** — unpublish→publish 직후 예측
   엔드포인트가 잠시 이전 모델을 서빙하는 전파 지연이 관측됐다(Iteration 2 게시 직후 예측값이
   Iteration 1 과 소수점까지 동일 → 수 분 뒤 재예측 시 실제 새 모델 값으로 바뀜). 게시 후 몇 분
   뒤에 재예측으로 확인할 것.
